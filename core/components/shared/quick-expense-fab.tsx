...
function QuickExpenseContent({
  ...
  envelopes,
  selectedEnvelope,
}: Omit<ReturnType<typeof useCreateExpense>, "limitLabel"> & {
  envelopes: NonNullable<ReturnType<typeof useEnvelopes>["envelopes"]>;
  ...
  return (
    <>
      ...
      <div
        className={cn("px-4 pb-6 space-y-4", isAtLimit ? "hidden" : undefined)}
      >
        {/* Envelope cards */}
...
export default function QuickExpenseFAB() {
  ...
  if (envelopes === undefined || envelopes === null) {
    return (
      <div className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-muted animate-pulse" />
    );
  }

  const title = (
    <span className="text-center text-lg flex items-center justify-center gap-2">
      Gasto rápido
      {limitLabel !== null ? (
        <span
          className={cn(
